...
export const REMOTE_ACCESS_POINT_API = "http://10.20.25.8:8000/api/v1/data-access-points"
...
function extractItems(payload: unknown): unknown[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    const candidateKeys = ["data", "items", "result", "access_points", "accessPoints"]
...
function toApiResponse(accessPoints: AccessPointItem[]) {
  return {
    status: "ok",
    accessPoints,
    groups: groupByBts(accessPoints),
  }
}
...
function sanitizeStatus(value: unknown): string {
  if (typeof value !== "string") {
    throw new Error("Status access point wajib diisi.")
  }
...
function parseUpsertPayload(body: Record<string, unknown>): UpsertPayload {
  const ssid = sanitizeRequiredString("Nama Access Point", body.ssid ?? body.nama_ap)
  const btsName = sanitizeRequiredString("Nama BTS", body.btsName ?? body.nama_bts)
  const routerName = sanitizeRequiredString("Router", body.routerName ?? body.router_ap)
  const interfaceName = sanitizeRequiredString("Interface", body.interfaceName ?? body.interface_ap)
  const ipAddress = sanitizeRequiredString("IP Address", body.ipAddress ?? body.ip_ap)
  const device = sanitizeRequiredString("Perangkat", body.device ?? body.perangkat_ap)
  const username = sanitizeRequiredString("Login", body.username ?? body.login_ap)
  const password = sanitizeRequiredString("Password", body.password ?? body.password_ap)
  const frequency = sanitizeRequiredString("Frequency", body.frequency ?? body.freq_ap)
  const channelWidth = sanitizeRequiredString("Channel Width", body.channelWidth ?? body.cw_ap)
  const macAddress = sanitizeRequiredString("MAC Address", body.macAddress ?? body.mac_ap)
  const status = sanitizeStatus(body.status ?? body.status_ap)
  const security = sanitizeOptionalString(body.security ?? body.security_ap)
  const phraseKey = sanitizeOptionalString(body.phraseKey ?? body.phrase_ap)
...
export async function GET() {
  try {
    ...
    console.error("[api/access-points] failed to fetch data", error)
    return NextResponse.json(
      {
        status: "error",
        message: "Gagal mengambil data access point.",
      },
      { status: 500 },
    )
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      {
        ...toApiResponse(accessPoints),
        message: "Access point berhasil ditambahkan.",
      },
      { status: 201 },
    )
  } catch (error) {
    console.error("[api/access-points] failed to create data", error)
    return NextResponse.json(
      {
        status: "error",
        message: error instanceof Error ? error.message : "Gagal menambahkan access point.",
      },
      { status: 400 },
    )
  }
...
export async function PUT(request: Request) {
  try {
    ...
    const id = typeof idRaw === "number" ? idRaw : Number(idRaw)
    if (!Number.isFinite(id) || id <= 0) {
      throw new Error("ID access point tidak valid.")
    }
    ...
    console.error("[api/access-points] failed to update data", error)
    return NextResponse.json(
      {
        status: "error",
        message: error instanceof Error ? error.message : "Gagal memperbarui access point.",
      },
      { status: 400 },
    )
  }
...
export async function DELETE(request: Request) {
  try {
    ...
    console.error("[api/access-points] failed to delete data", error)
    return NextResponse.json(
      {
        status: "error",
        message: error instanceof Error ? error.message : "Gagal menghapus access point.",
      },
      { status: 400 },
    )
  }
